...
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(__file__))) # Goes up to Root
...
def run():
    ...
    train_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'target_train.csv'), num_items, num_negatives=4, is_training=True),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=0
    )
    
    valid_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'target_valid.csv'), num_items, is_training=False),
    ...
    for epoch in range(EPOCHS):
        ...
        for users, pos_items, neg_items in train_loop:
            ...
            users_expanded = users.unsqueeze(1).repeat(1, neg_items.size(1)).view(-1)
        ...
        if metrics['recall'] > best_hr:
            ...
            torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "best_ncf_model.pth"))
    ...
    model.load_state_dict(torch.load(os.path.join(OUTPUT_DIR, "best_ncf_model.pth")))
    
    test_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'target_test.csv'), num_items, is_training=False),
    ...
    print(f"COLD START TEST: R@10 = {test_recall:.4f}, P@10 = {test_precision:.4f}, f1@10 = {test_f1:.4f}, NDCG@10 = {test_ndcg:.4f}, MAP = {test_map:.4f}")
    
    # Save Final Result
    final_results = {"test_recall": test_recall, "test_precision": test_precision, "test_f1": test_f1, "test_ndcg": test_ndcg, "test_map": test_map}
