...
def run():
    """
    Runs the training pipeline for the CMF (Cross-Domain Matrix Factorization) model.

    Steps:
    1. Loads dataset dimensions and files.
    2. Initializes DataLoaders for target train, source train (auxiliary), and target validation.
    3. Initializes the CMF model, optimizer, and loss function.
    4. Runs the training loop:
        - Updates target domain embeddings.
        - Updates source domain embeddings (auxiliary task).
        - Combines losses.
    ...
    tgt_train_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'target_train.csv'),
                  num_tgt_items, num_negatives=4, is_training=True),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=0
    )

    # Auxiliary Loader (Source) - Used to refine User Embeddings
    src_train_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'source_train.csv'),
                  num_src_items, num_negatives=4, is_training=True),
        batch_size=BATCH_SIZE, shuffle=True, num_workers=0
    )
    ...
    valid_loader = DataLoader(
        CDDataset(os.path.join(DATA_DIR, 'target_valid.csv'),
                  num_tgt_items, is_training=False),
    ...
    for epoch in range(EPOCHS):
        ...
        train_loop = tqdm(tgt_train_loader,
                          desc=f"Epoch {epoch+1}/{EPOCHS}", leave=False)

        for users_t, pos_t, neg_t in train_loop:
            users_t, pos_t, neg_t = users_t.to(
                DEVICE), pos_t.to(DEVICE), neg_t.to(DEVICE)
            ...
            users_t_exp = users_t.unsqueeze(
                1).repeat(1, neg_t.size(1)).view(-1)
            ...
            users_s, pos_s, neg_s = users_s.to(
                DEVICE), pos_s.to(DEVICE), neg_s.to(DEVICE)
            ...
            users_s_exp = users_s.unsqueeze(
                1).repeat(1, neg_s.size(1)).view(-1)
            neg_preds_s = model(users_s_exp, neg_s_flat, domain='source')
            loss_src_neg = criterion(
                neg_preds_s, torch.zeros_like(neg_preds_s))
    ...
    print(
        f"COLD START TEST: R@10 = {test_recall:.4f}, NDCG@10 = {test_ndcg:.4f}, MAP = {test_map:.4f}")

    final_results = {"test_recall": test_recall, "test_ndcg": test_ndcg,
                     "test_map": test_map, "test_precision": test_precision, "test_f1": test_f1}
