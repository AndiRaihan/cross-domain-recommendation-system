...
def run_ptupcdr():
    ...
    history = {"gcn_loss": [], "gcn_val_recall": [], "meta_loss": [], "meta_val_recall": []}
    ...
    # Pass counts explicitly to fix the RuntimeError
    adj_t = build_adjacency_matrix(DATA_DIR, 'target_train.csv', num_users, num_tgt_items)
    
    model_t = LightGCN(num_users, num_tgt_items, adj_t, tgt_feats, embed_dim=EMBED_DIM, device=DEVICE).to(DEVICE)
    ...
    if os.path.exists(target_model_path) and not FORCE_RETRAIN:
        ...
        loader_t = DataLoader(CDDataset(os.path.join(DATA_DIR, 'target_train.csv'), num_tgt_items, num_negatives=1), batch_size=BATCH_SIZE, shuffle=True)
        valid_loader_t = DataLoader(CDDataset(os.path.join(DATA_DIR, 'target_valid.csv'), num_tgt_items, is_training=False), batch_size=128)

        best_gcn_hr = 0.0
        for epoch in range(EPOCHS_GCN):
            model_t.train()
            total_loss = 0
            loop = tqdm(loader_t, desc=f"Target Epoch {epoch+1}/{EPOCHS_GCN}", leave=False)
            for u, p, n in loop:
                ...
                loss = -torch.mean(torch.log(torch.sigmoid(pos_scores.unsqueeze(1) - neg_scores) + 1e-10))
                opt_t.zero_grad()
                loss.backward()
                opt_t.step()
            ...
            class GCNWrapper(torch.nn.Module):
                ...
                def forward(self, u, i): ue, ie = self.model(); return (ue[u] * ie[i]).sum(dim=1)
                    
            metrics = evaluate_model(GCNWrapper(model_t), valid_loader_t, device=DEVICE)
            print(f"   Epoch {epoch+1}: Loss {total_loss:.2f} | Val Recall {metrics['recall']:.4f}")
        ...
        torch.save(model_t.state_dict(), os.path.join(OUTPUT_DIR, "best_target_gcn.pth"))
    ...
    # Pass counts explicitly here too
    adj_s = build_adjacency_matrix(DATA_DIR, 'source_train.csv', num_users, num_src_items)
    
    model_s = LightGCN(num_users, num_src_items, adj_s, src_feats, embed_dim=EMBED_DIM, device=DEVICE).to(DEVICE)
    ...
    if os.path.exists(source_model_path) and not FORCE_RETRAIN:
        ...
        loader_s = DataLoader(CDDataset(os.path.join(DATA_DIR, 'source_train.csv'), num_src_items), batch_size=BATCH_SIZE, shuffle=True)
        
        for epoch in range(5): 
            model_s.train()
            total_loss = 0
            loop = tqdm(loader_s, desc=f"Source Epoch {epoch+1}/5", leave=False)
            for u, p, n in loop:
                u, p, n = u.to(DEVICE), p.to(DEVICE), n.to(DEVICE)
                u_e, i_e = model_s.forward()
                loss = -torch.mean(torch.log(torch.sigmoid((u_e[u]*i_e[p]).sum(1).unsqueeze(1) - (u_e[u].unsqueeze(1)*i_e[n]).sum(2)) + 1e-10))
                opt_s.zero_grad()
                loss.backward()
                opt_s.step()
                total_loss += loss.item()
    ...
    if os.path.exists(mapper_model_path) and not FORCE_RETRAIN:
        ...
        # Note: If you load the mapper, you skip the training loop, 
        # but you might want to run validation/metrics anyway.
    else:
        df_train = pd.read_csv(os.path.join(DATA_DIR, 'target_train.csv'))
        bridge_train = df_train[df_train['is_overlap'] == True]['user_id_idx'].unique()
        bridge_train_tensor = torch.tensor(bridge_train, dtype=torch.long)
        
        opt_m = optim.Adam(mapper.parameters(), lr=LR_META)
        map_loader = DataLoader(TensorDataset(bridge_train_tensor), batch_size=256, shuffle=True)
        
        df_val = pd.read_csv(os.path.join(DATA_DIR, 'target_valid.csv'))
        val_users_tensor = torch.tensor(df_val['user_id_idx'].unique(), dtype=torch.long).to(DEVICE)
        best_meta_recall = 0.0

        # Wrapper
        class HybridEvaluator(torch.nn.Module):
            def __init__(self, u_emb, i_emb): super().__init__(); self.u_emb = u_emb; self.i_emb = i_emb
            def forward(self, u, i): return (self.u_emb[u] * self.i_emb[i]).sum(dim=1)
        ...
        for epoch in range(EPOCHS_META):
            mapper.train()
            total_loss = 0
            loop = tqdm(map_loader, desc=f"Meta Epoch {epoch+1}/{EPOCHS_META}", leave=False)
            ...
            mapper.eval()
            with torch.no_grad():
                fake_tgt_val = mapper(emb_s[val_users_tensor], user_content_tensor[val_users_tensor])
                hybrid_emb = emb_t.clone()
                hybrid_emb[val_users_tensor] = fake_tgt_val
                metrics = evaluate_model(HybridEvaluator(hybrid_emb, final_items_t), valid_loader_t, device=DEVICE)
                
            print(f"   Epoch {epoch+1}: MSE {total_loss:.4f} | Val Recall {metrics['recall']:.4f}")
    ...
    with torch.no_grad():
        ...
        test_loader = DataLoader(CDDataset(os.path.join(DATA_DIR, 'target_test.csv'), num_tgt_items, is_training=False), batch_size=100)
