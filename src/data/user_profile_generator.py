...
def generate_user_profiles(data_dir: str, domain_prefix: str):
    ...
    item_emb = np.load(os.path.join(
        data_dir, f'{domain_prefix}_domain_mrl.npy'))
    ...
    user_content_emb = np.zeros(
        (num_users_global, embed_dim), dtype=np.float32)
    ...
    print(
        f"Saved {domain_prefix} Matrix: {user_content_emb.shape} (Active Users: {found_count})")
