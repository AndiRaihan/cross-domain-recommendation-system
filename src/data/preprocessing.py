...
class Preprocessor:
    ...
    def align_and_map(self, df_source: pd.DataFrame, df_target: pd.DataFrame):
        ...
        # Global User Mapping (Union of both domains)
        unique_users = pd.concat(
            [df_source['user_id'], df_target['user_id']]).unique()
        # Start ID at 1 for RecBole
        ...
        df_target['is_overlap'] = df_target['user_id'].apply(
            lambda x: x in overlap)
    ...
    def generate_statistics(self, df_s: pd.DataFrame, df_t: pd.DataFrame):
        ...
        with open(os.path.join(self.save_dir, 'dataset_statistics.txt'), 'w') as f:
            ...
            f.write(
                f"Avg Actions/User: {stats_s['avg_interactions_per_user']}\n\n")
            ...
            f.write(
                f"Avg Actions/User: {stats_t['avg_interactions_per_user']}\n\n")

            f.write("--- CROSS-DOMAIN BRIDGE ---\n")
            f.write(f"Overlapping Users: {n_overlap}\n")
            f.write(
                f"Coverage in Target: {overlap_stats['overlap_percentage_of_target']}%\n")
            f.write(
                "(This % represents the users we can help using Transfer Learning)\n")
    ...
    def export_data(self, df: pd.DataFrame, name: str, folder: str, implicit_threshold: int = 0):
        ...
        if implicit_threshold > 0:
            print(
                f"   [Implicit] Filtering {name} ratings < {implicit_threshold}...")
        ...
        cols = ['user_id_idx', 'item_id_idx',
                'label', 'timestamp', 'is_overlap']
    ...
    def process(self):
        ...
        # 3. Align
        df_s, df_t, user_map, item_map_s, item_map_t = self.align_and_map(
            df_s, df_t)
        ...
        self.export_data(df_s, "source_domain",
                         self.save_dir, implicit_threshold=0)
        self.export_data(df_t, "target_domain",
                         self.save_dir, implicit_threshold=0)
...
if __name__ == "__main__":
    ...
    runner = Preprocessor(
        SOURCE, TARGET, "C:\\Tugas Raihan\\Latihan Python\\cross-domain-recommendation-system\\data\\processed")
    runner.process()
