...
class FeatureEngineer:
    ...
    def parse_details(self, details_obj):
        """Extracts specific high-signal keywords from details dict."""
        if not isinstance(details_obj, dict):
            return ""
        
        keep_keywords = ['content advisory', 'label', 'producer', 'director', 'starring', 'supporting actors']
        extracted_text = []
    ...
    def construct_text(self, item):
        ...
        if len(desc_str) > 500: desc_str = desc_str[:500] 
    ...
    def process_domain(self, meta_file_path, mapping_file, output_name, batch_size=64):
        print(f"\n--- Processing {output_name} ---")
        
        if (os.path.exists(os.path.join(self.data_dir, f"{output_name}_feat.npy"))):
            print(f"Features Already Exist, Skipping {os.path.join(self.data_dir, f'{output_name}_feat.npy')}")
            return
        ...
        if not valid_asins:
            raise ValueError(f"Mapping file {mapping_file} is empty or invalid!")
        ...
        embedding_matrix = np.zeros((num_items, self.embedding_dim), dtype=np.float16)
                ...
                for chunk in reader:
                    ...
                    for _, row in relevant_rows.iterrows():
                        ...
                        if len(batch_texts) >= batch_size:
                            self._encode_and_store(batch_texts, batch_indices, embedding_matrix)
        ...
        if found_count == 0:
            print("[WARNING] Found 0 items! Check if 'parent_asin' vs 'asin' matches your metadata file.")
    ...
    def _encode_and_store(self, texts, indices, matrix):
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    engineer = FeatureEngineer(DATA_DIR, model_name='Qwen/Qwen3-Embedding-0.6B')
    
    # Run Source
    engineer.process_domain(META_SOURCE, 'source_item_mapping.json', 'source_domain', batch_size=64)
    
    # Run Target
    engineer.process_domain(META_TARGET, 'target_item_mapping.json', 'target_domain', batch_size=64)