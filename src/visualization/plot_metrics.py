...
def plot_ncf_failure():
    ...
    ax2.plot(epochs, recall, color=color, linewidth=2, linestyle='--', label='Recall@10')
    ax2.tick_params(axis='y', labelcolor=color)

    plt.title('The Cold-Start Paradox: Overfitting to Warm Users', fontsize=14)
    fig.tight_layout()
    plt.savefig(os.path.join(REPORTS_DIR, "figures", "viz1_ncf_failure.png"), dpi=300)
    print("Saved Viz 1.")

def plot_model_comparison():
    """Viz 2: The Transfer Lift Bar Chart"""
    # Load final results
    results = {}
    ...
    for name, path in paths.items():
        if os.path.exists(path):
            ...
            labels.append(name)
            for m in metrics: data[m].append(0)
    ...
    rects1 = ax.bar(x - width, data['recall'], width, label='Recall@10', color='#4e79a7')
    rects2 = ax.bar(x, data['ndcg'], width, label='NDCG@10', color='#f28e2b')
    rects3 = ax.bar(x + width, data['map'], width, label='MAP', color='#e15759')

    ax.set_ylabel('Score')
    ax.set_title('Cross-Domain Model Comparison (Cold-Start Test Set)', fontsize=14)
    ax.set_xticks(x)
    ...
    plt.savefig(os.path.join(REPORTS_DIR, "figures", "viz2_comparison.png"), dpi=300)
    print("Saved Viz 2.")
